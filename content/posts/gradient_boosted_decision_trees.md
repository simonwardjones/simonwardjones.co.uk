...
In order to give intuition it is easiest to consider first the case of regression. Imagine we are again trying to predict house prices in a desirable area of north London. With training data that looks the following
...
Now we have a second prediction $F_1$ we can continue in a sequential manner, again calculating the error of our second prediction $e_2$ for the training data and then training a tree $f_2$ to predict this second error. Then once again we add this second predicted error to the second prediction to get a third prediction $F_2 = F_1 + f_2$ and so on. As the models are summed together this approach is known as an `aditive model`. In general we have 
$$F_m =  F_{m-1} + f_m$$ 
...
3. Calculate new prediction $F_1 = f_0 + \mu * f_1$ where $\mu$ is the learning rate
4. Repeat 2, 3 as many times as chosen where in general
    1. Train model $f_m$ on the error $e_m = y - F_{m-1}$
    2. Calculate new prediction as $F_{m-1} + \mu * f_m$
...
In the following section we explore the mathematical details and extend the algorithm to the classification setting.
...
In general in `supervised learning` we aim to find a model $F$ to fit the data such that the predicted value $\hat{y}^j$ for the $j$th training example $\mathbf{x}^j$ is approximately equal to the $j$th target value $y^j$ or equivalently


{{<formula class="responsive-math">}}
\hat{y}^j=F(\mathbf{x}^j)\sim y^j \quad\forall j \in {1,\dots,n} 
{{</formula>}}
...
In a `parametric` setting (e.g. logistic regression) the model can be written as $\hat{y}^j=F_{\mathbf{\theta}}(\mathbf{x}^j)$. Where the subscript $\mathbf{\theta}$ indicates the models dependence on the parameters. We can also write the loss in terms of $\mathbf{\theta}$ as $\mathcal{L(y, \hat{y}(\mathbf{\theta})})$. In this setting we update the model parameters using gradient descent. That is we iteratively update the model parameters by stepping the parameters in the direction of the negative gradient of the loss function with respect to the parameters (where $\mu$ is the learning rate).

{{<formula class="responsive-math">}}
\mathbf{\theta}^{m+1} = \mathbf{\theta}^{m} - \mu* \frac{\partial\mathcal{L}}{\partial{\mathbf{\theta}^m}}
{{</formula>}}
...
{{<formula class="responsive-math">}}
\hat{y}^j \to \hat{y}^j - \eta\frac{\partial\mathcal{L}}{\partial{\hat{y}^j}}
{{</formula>}}

Equivalently we update $F_{m-1}$ by adding another "delta model" $f_{m+1}$

{{<formula class="responsive-math">}}
\hat{y}^j = F_m(\mathbf{x}^j) + f_{m+1}(\mathbf{x}^j) \quad\forall j \in {1,\dots,n} 
{{</formula>}}

Where $\eta$ is the learning rate and
{{<formula class="responsive-math">}}
f_{m+1}(\mathbf{x}^j)= -\eta\frac{\partial\mathcal{L}}{\partial{\hat{y}^j}}
{{</formula>}}

In practise we cannot set this delta model exactly so we train a model on the data to fit $ - \mu\frac{\partial\mathcal{L}}{\partial{\hat{y}^j}}$. In general this gradient can be fitted with any model but gradient boosted decision trees use a decision tree - hence the name! Note each tree will have it's own Loss $\mathcal{L}^{f_{m+1}}$ separate to the global loss $\mathcal{L}$.
...
$f_0(x) = \log\left(\frac{\sum{y_i\in C_1}}{n}\right)$ then set $F_0(x) = f_0(x)$  
...
How the process looks like the following:
...
Then sequentially fit $f_1^k,f_2^k,f_3^k,\dots, f_m^k$ against the residuals for each class $k$ to calculate additive models $F_1,F_2,F_3,\dots, F_m$
such that $\forall k\in\{1,\dots, K\}$
{{<formula class="responsive-math">}}
\begin{align}
f_1(x)^k&\sim (y^k-\sigma_k(F_0^1,\dots,F_0^K))\\
F_1(x)^k&= F_0(x)^k + \eta f_1(x)^k \\
f_2(x)^k&\sim (y^k-\sigma_k(F_1^1,\dots,F_1^K))\\
F_2(x)^k&= F_1(x)^k + \eta f_2(x)^k \\
\quad&\vdots\\
f_m(x)^k&\sim (y^k-\sigma_k(F_{m-1}^1,\dots,F_{m-1}^K))\\
F_m(x)^k&= F_{m-1}(x)^k + \eta f_m(x)^k\\
\end{align}
{{</formula>}} 
