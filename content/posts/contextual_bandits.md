...
a_t &= \underset{a}{\operatorname{argmax}}(x_{t,a}^T \theta_a + \alpha \sqrt{x_{t,a}^T (D_a^T D_a + I)^{-1} x_{t,a}}) \\
    &= \underset{a}{\operatorname{argmax}}(x_{t,a}^T \theta_a + \alpha \sqrt{x_{t,a}^T A_a x_{t,a}})
\end{align}
{{</formula>}}

where $A_a = (D_a^T D_a + I)^{-1}$
...
- For t = 1, 2, ... , T
  ...
  - For each arm $a \in A_t$
    ...
    - Update current mean and variance estimates
      - $\theta_a = A_a^{-1} b_a$
      - $p_a = x_{t,a}^T \theta_a + \alpha \sqrt{x_{t,a}^T A_a x_{t,a}}$
...
The below chart shows the true conversion parameter $\theta_k$ for each arm or article plotted against the age of the user. The conversion parameter is the probability of the user clicking on the article given their age.
